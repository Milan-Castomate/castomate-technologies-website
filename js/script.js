document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadMegaMenu() {
        ...
        if (!megaMenuContainer) return; // Exit if the menu container isn't on the page
        ...
        try {
            ...
            categories.forEach(category => {
                ...
                // Link to the products page, with the category ID as a hash to scroll to it
                ...
                categoryItem.addEventListener('mouseenter', () => {
                    ...
                    if (category.products && category.products.length > 0) {
                        const productsToShow = category.products.slice(0, 9); // Limit to showing 9 products in the menu
                        productsToShow.forEach(product => {
                            const productItem = document.createElement('li');
                            productItem.classList.add('product-link');
                            // This link will go to the main products page and trigger the modal via the hash
            ...
            // Trigger a hover on the first category to show its products by default
    ...
    loadMegaMenu(); // Call the function to build the menu on every page load
    ...
    if(heroSlider) {
        ...
        if (slides.length > 0) {
            ...
            if (prevSlideBtn && nextSlideBtn) {
                ...
                nextSlideBtn.addEventListener('click', () => { nextSlideFn(); resetSlideInterval(); });
            }
            if (slides.length > 1) {
                 slideInterval = setInterval(nextSlideFn, 7000);
            }
