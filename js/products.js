document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchProducts() {
        try {
            ...
            // Create a flat array of all products for easier searching and linking
            allProductsFlat = [];
            ...
            // Only run page-specific logic if the necessary containers exist
            if (categoryTabsContainer) {
                populateCategoryTabs(allCategoriesData);
                filterAndSearchProducts(); // Initial display
                handleUrlHash(); // Check for and handle a #product-id in the URL
            }

            // Populate the dropdown on the "Request a Quote" page if it exists
    ...
    /**
     * Checks for a hash in the URL and opens the corresponding product modal.
     */
    function handleUrlHash() {
        if (window.location.hash) {
            const productId = window.location.hash.substring(1); // Remove '#'
            const product = allProductsFlat.find(p => p.id === productId);
            if (product) {
                // Use a short timeout to ensure the page has rendered before opening the modal
                setTimeout(() => {
    ...
    function populateCategoryTabs(categories) {
        ...
        categoryTabsContainer.innerHTML = ''; // Clear "Loading..."
    ...
    function displayProductsByCategory(categoriesToDisplay) {
        ...
        categoriesToDisplay.forEach(category => {
            // Only create a section if the category has products to display
    ...
    function filterAndSearchProducts() {
        ...
        if (searchTerm) {
            ...
            if (searchedProducts.length === 0) {
                productListContainer.innerHTML = `<p class="no-results-message">No products found matching: "${productSearchInput.value}"</p>`;
                return;
            }
            ...
            searchedProducts.forEach(product => {
                if (!groupedBySearch[product.categoryId]) {
                    ...
                    groupedBySearch[product.categoryId] = {
                        ...
                        categoryName: originalCategory ? originalCategory.categoryName : 'Search Results',
                        products: []
                    };
                }
