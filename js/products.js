document.addEventListener('DOMContentLoaded', () => {
    ...
    function filterAndSearchProducts() { const searchTerm = productSearchInput ? productSearchInput.value.toLowerCase().trim() : ''; let categoriesToDisplay; let operationType = 'filter'; if (searchTerm) { operationType = 'search'; const searchedProducts = allProductsFlat.filter(product => product.name.toLowerCase().includes(searchTerm) || (product.details.application && product.details.application.toLowerCase().includes(searchTerm))); if (searchedProducts.length === 0) { productListContainer.innerHTML = `<p class="no-results-message">No products found matching: "${productSearchInput.value}"</p>`; return; } const groupedBySearch = {}; searchedProducts.forEach(product => { if (!groupedBySearch[product.categoryId]) { const originalCategory = allCategoriesData.find(cat => cat.categoryId === product.categoryId); groupedBySearch[product.categoryId] = { categoryId: product.categoryId, categoryName: originalCategory ? originalCategory.categoryName : 'Search Results', products: [] }; } groupedBySearch[product.categoryId].products.push(product); }); categoriesToDisplay = Object.values(groupedBySearch); } else { operationType = 'filter'; if (currentFilterCategoryId === 'all') { categoriesToDisplay = allCategoriesData; } else { const selectedCat = allCategoriesData.find(category => category.categoryId === currentFilterCategoryId); categoriesToDisplay = selectedCat ? [selectedCat] : []; } } const hasProductsToDisplay = categoriesToDisplay && categoriesToDisplay.some(category => category.products && category.products.length > 0); if (!hasProductsToDisplay) { if (operationType === 'search') { productListContainer.innerHTML = `<p class="no-results-message">No products found matching: "${productSearchInput.value}"</p>`; } else if (currentFilterCategoryId !== 'all') { productListContainer.innerHTML = `<p class="no-results-message">No products currently listed in this category.</p>`; } else { productListContainer.innerHTML = `<p class="no-results-message">No products available at the moment.</p>`; } } else { displayProductsByCategory(categoriesToDisplay); } }
